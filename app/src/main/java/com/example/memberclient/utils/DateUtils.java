...
//                ToastUtil.Toast("所选日期不能不能大于或等于当前日期");
     ...
     * 获取当前日期  2017年8月5日
     * */
    public static Date getCurDate(String str,String format) {
        SimpleDateFormat sdf = new SimpleDateFormat(format);
        Date today = null;
        try {
            today = sdf.parse(str);
     ...
     * 是否输入当前月
     */
    public static boolean isBelongCurrentMonth(String time1,String year, String month) {
        String startStr = year + "-" + month + "-00 00:00:00";
        Date date = DateUtils.str2Date(startStr);
        Date endDate = DateUtils.addMonth(date, 1);
        Date currentDate=new Date(Long.parseLong(time1));


        return currentDate.getTime()>=date.getTime()&& currentDate.getTime()<=endDate.getTime();
    }
